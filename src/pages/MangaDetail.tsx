...
export function MangaDetail({ mangaId }: MangaDetailProps) {
  ...
  const [singleCacheProgress, setSingleCacheProgress] = useState({ current: 0, total: 0 });
      ...
      try {
        ...
        if (isPluginManga && pluginKey && comicId) {
          ...
          if (cached) {
            ...
            console.log("[MangaDetail] Saved to cache");
          }
  ...
  useEffect(() => {
    const checkCacheStatus = async () => {
      ...
      // 从缓存索引中读取已缓存的章节
      const cachedChapterIds = await cacheIndex.getCachedChapterIds(manga.externalId);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (detail.chapters) {
        ...
        const chapterList = await ChapterList.findOne({ where: { mangaId: manga.id } });
        if (chapterList) {
          // 合并阅读状态
    ...
    const unsubscribe = cacheQueue.subscribe((tasks) => {
      ...
      if (task) {
        if (task.status === 'running') {
          setIsCaching(true);
          const progress = task.progress.totalImages > 0
            ? Math.round(((task.progress.cachedImages + task.progress.skippedImages) / task.progress.totalImages) * 100)
            : 0;
          setCacheProgress(progress);
          setCacheStatus(`队列中: ${task.progress.currentChapter}/${task.progress.totalChapters} 章 (${task.progress.cachedImages} 张)`);
  ...
  const handleDeleteChapterCache = async (chapter: MemoryChapter) => {
    ...
    // 删除文件缓存
    const cacheQueue = getCacheQueue();
    const success = await cacheQueue.deleteChapterCache(manga.externalId, chapter.id);
    ...
    try {
      ...
      if (chapterList) {
        ...
        const updatedChapters = chapters.map(ch =>
          ch.id === chapter.id
            ? { ...ch, isRead: !ch.isRead, readAt: !ch.isRead ? Date.now() : undefined }
            : ch
        );
      ...
      <div class="mt-20 px-4">
        ...
        <div class="mt-6">
                ...
                >
                  {isCaching ? (
                    <>
                      ...
                      <span class="text-[10px] opacity-75 truncate max-w-[100px]">{cacheStatus}</span>
                    </>
                  ) : (
                    <>
                      <span class="flex items-center gap-1">
          ...
          <div class="mt-3 space-y-1">
            {getSortedChapters().slice(0, showAllChapters ? undefined : 10).map((chapter) => (
              <div
                key={chapter.id}
                      ...
                      {cachedChapters.has(chapter.id) ? (
                        ...
                        >
                          {cachingChapterId === chapter.id ? (
                            <>
                              <span class="w-3 h-3 border-2 border-current border-t-transparent rounded-full animate-spin" />
                              <span class="text-[8px]">{singleCacheProgress.current}/{singleCacheProgress.total}</span>
                            </>
                          ) : (
                            '💾'
                          )}
